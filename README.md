...
- **Повторяет при:** сетевых сбоях (`connection refused`, `timeout`), `429 Too Many Requests`, `5xx Server Error`
- **Не повторяет при:** `2xx`, `3xx`, `4xx` (кроме 429) — считаются окончательным результатом
...
При отмене контекста (`Ctrl+C` или `context.WithTimeout`) ожидание между попытками немедленно прерывается. Итоговый статус последней попытки фиксируется в отчёте (`status: "error"` при исчерпании лимита).
